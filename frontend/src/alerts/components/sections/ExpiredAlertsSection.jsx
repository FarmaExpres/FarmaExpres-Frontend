...
import { formatDaysLabel } from '../../utils/alertsFormatters'
...
const columns = [
  ...
  { key: 'dias', label: 'DÍAS', className: 'whitespace-nowrap font-semibold text-[#2e4268]', value: (row) => formatDaysLabel(row.diasRestantes) },
  {
    key: 'stock',
    label: <span title="Stock del lote vencido">STOCK VENC.</span>,
    className: 'whitespace-nowrap font-semibold tabular-nums text-[#364b75]',
    value: (row) => row.stock
  }
]

const ExpiredAlertsSection = ({ section, rows, isOpen, isLoading, onToggle }) => (
  <AlertSectionCard
...
export default ExpiredAlertsSection
