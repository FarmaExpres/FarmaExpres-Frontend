...
import { toRestockSuggestion } from '../../utils/alertsFormatters'
...
const columns = [
  ...
  { key: 'stock', label: 'STOCK', className: 'whitespace-nowrap font-semibold tabular-nums text-red-700', value: (row) => row.stock },
  { key: 'minimo', label: 'MÍNIMO', className: 'whitespace-nowrap font-semibold tabular-nums text-[#364b75]', value: (row) => row.stockMinimo },
  { key: 'reponer', label: 'REPOSICIÓN SUGERIDA', className: 'whitespace-nowrap font-semibold text-[#7c3aed]', value: (row) => toRestockSuggestion(row) }
]

const OutOfStockAlertsSection = ({ section, rows, isOpen, isLoading, onToggle }) => (
  <AlertSectionCard
...
export default OutOfStockAlertsSection
